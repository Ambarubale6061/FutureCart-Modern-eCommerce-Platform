...
import { Mail, Lock, Sparkles, Shield, Store, User } from "lucide-react";

type TabType = "user" | "admin" | "seller";
...
const Login = () => {
  ...
  const tabs = [
    { id: "user" as TabType, label: "User", icon: User, gradient: "from-primary via-primary to-accent", desc: "Access your FutureCart account" },
    { id: "admin" as TabType, label: "Admin", icon: Shield, gradient: "from-red-600 via-red-500 to-orange-500", desc: "Admin Dashboard Access" },
                ...
                <motion.div key={activeTab} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                  <div className={`bg-gradient-to-br ${activeTabData.gradient} p-8 text-center`}>
                    ...
                    <h2 className="text-2xl font-bold text-white">
                      {activeTab === "user" ? "Welcome Back" : activeTab === "admin" ? "Admin Login" : "Seller Portal"}
                    </h2>
                    ...
                    {activeTab === "seller" ? (
                      ...
                      <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <motion.button whileTap={{ scale: 0.98 }} type="submit" disabled={isLoading}
                          className={`w-full rounded-xl py-3.5 text-sm font-bold shadow-lg transition-all disabled:opacity-50 ${
                            activeTab === "admin"
